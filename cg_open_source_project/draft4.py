...
# Function to draw a pumpkin-like sphere with shading and light
def draw_pumpkin(context, center_x, center_y, radius):
    ...
    # Create vertical ridges (for the pumpkin texture)
    num_ridges = 10
    for i in range(num_ridges):
        angle = (i / num_ridges) * 2 * math.pi
        ridge_x = center_x + radius * 0.9 * math.cos(angle)
        ridge_y = center_y + radius * 0.9 * math.sin(angle)
        context.move_to(center_x, center_y)
        context.line_to(ridge_x, ridge_y)
        context.set_source_rgba(0, 0, 0, 0.2)  # Light shadow effect for the ridges
        context.stroke()
...
# Save the image
surface.write_to_png("pumpkin4.png")

print("3D glowing pumpkin image created!")
